...
$(function () {
  ...
  $(".menu__item--plus, .menu__dropdown").hover(
    function () {
      clearTimeout(timer);
      $(this).addClass("drop");
    },
  ...
  const removeTo = function (selector) {
      let value = $(selector).attr("data-value");
      value--;
      $(selector).attr("data-value", value).text(value);
  };
  ...
  $(".products__button--love, .products__icon--love").on("click", function (e) {
    e.preventDefault();
    let target = $(e.currentTarget).attr('data-value');
    if (target == 0) {
    target++
    $(e.currentTarget).attr('data-value', target);
    addTo(".user-nav__num--love");
    Swal.fire("", "Added to wishlist!", "success");
    } else {
    target--
    $(e.currentTarget).attr('data-value', target);
    removeTo(".user-nav__num--love");
    Swal.fire("", "Deleted from wishlist!", "success");
    }
  ...
  $(".user-nav__layout--3").on("click", function () {
    Swal.fire("This is demo", "The options button!", "success");
  });
  ...
  $(".related__slider").slick({
    ...
    prevArrow:
      '<button type="button"class="related__btn related__prev"></button>',
    nextArrow:
      '<button type="button"class="related__btn related__next related__btn--active"></button>',
    responsive: [
      {
        breakpoint: 426,
        settings: {
